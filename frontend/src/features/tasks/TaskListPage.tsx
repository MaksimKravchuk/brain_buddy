...
export function TaskListPage({ mode }: { mode?: "state" | "project" | "tag" }): React.JSX.Element {
  ...
  // Only the tasks actually on screen are asked about, and only when the relay
  // is rolled out for this user. A failed summary fetch degrades to no chips
  // rather than an error: the list itself is still perfectly usable.
  const relayEnabled = hasFeatureFlag(useAuthStore((store) => store.user), "external_agent_relay");
  // Not memoised: React Query hashes the key structurally, so a fresh array
  // identity each render does not refetch.
  const agentRunSummaries =
    useAgentRunSummaries(tasks.map((task) => task.id), relayEnabled).data ?? {};
