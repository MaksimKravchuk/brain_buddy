...
describe("agentHooks", () => {
  ...
  it("namespaces every agent query under one stable key factory", () => {
    ...
    expect(agentKeys.connections()).toEqual(["agents", "connections"]);
    expect(agentKeys.connection("conn-1")).toEqual(["agents", "connections", "conn-1"]);
    expect(agentKeys.runs("task-1")).toEqual(["agents", "runs", "task-1"]);
    expect(agentKeys.run("run-1")).toEqual(["agents", "run", "run-1"]);
    expect(agentKeys.summaries(["task-1"])).toEqual(["agents", "summaries", ["task-1"]]);
  });
