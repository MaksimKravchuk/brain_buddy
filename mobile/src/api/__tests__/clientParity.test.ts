...
const adapters = {
  ...
  listAgentConnections: (c) => c.listAgentConnections(), createAgentConnection: (c) => c.createAgentConnection({ name: "agent", endpoint_url: "https://agent.test", credential: "credential", current_password: "password" }, "key-agent-create"), getAgentConnection: (c) => c.getAgentConnection("connection-1"), updateAgentConnection: (c) => c.updateAgentConnection("connection-1", { expected_revision: 1 }, "key-agent-update"), testAgentConnection: (c) => c.testAgentConnection("connection-1"), rotateAgentCredential: (c) => c.rotateAgentCredential("connection-1", { credential: "credential", current_password: "password", expected_revision: 1 }, "key-agent-credential"), rotateAgentSigningSecret: (c) => c.rotateAgentSigningSecret("connection-1", { current_password: "password", expected_revision: 1 }, "key-agent-secret"), disconnectAgentConnection: (c) => c.disconnectAgentConnection("connection-1", { current_password: "password", expected_revision: 1 }, "key-agent-disconnect"), previewAgentHandoff: (c) => c.previewAgentHandoff("task-1", { connection_id: "connection-1" }), confirmAgentHandoff: (c) => c.confirmAgentHandoff("task-1", { connection_id: "connection-1", manifest_token: "token" }, "key-agent-confirm"), listAgentRuns: (c) => c.listAgentRuns("task-1"), getAgentRun: (c) => c.getAgentRun("run-1"), listAgentRunSummaries: (c) => c.listAgentRunSummaries(["task-1", "task-2"]), replyToAgentRun: (c) => c.replyToAgentRun("run-1", { message: "reply", expected_revision: 1 }, "key-agent-reply"), cancelAgentRun: (c) => c.cancelAgentRun("run-1", "key-agent-cancel"),
} satisfies Record<OperationName, Adapter>;

describe("common client wire parity inventory", () => {
  it("covers exactly 42 common operations with typed adapters and shared credentials", async () => {
