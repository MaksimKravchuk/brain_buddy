...
export function TaskListScreen({ state }: { state: Exclude<TaskState, "completed" | "cancelled"> }) {
  ...
  const tasks = useQuery({ queryKey: ["tasks", state], queryFn: () => api.tasks(state) });
  const capture = async () => {
    if (!draft.trim()) return;
    await api.createTask({ title: draft.trim(), state });
    setDraft("");
  ...
  return <View style={styles.page}>
    ...
    {tasks.isError ? <Text accessibilityRole="alert" style={styles.error}>Tasks could not be refreshed. Pull down to try again.</Text> : null}
    <FlatList data={tasks.data?.items ?? []} keyExtractor={(item) => item.id} onRefresh={() => void tasks.refetch()} refreshing={tasks.isFetching} contentContainerStyle={styles.list} ListEmptyComponent={!tasks.isLoading ? <Text style={styles.empty}>Nothing here yet.</Text> : null} renderItem={({ item }) => <Pressable accessibilityRole="button" onPress={() => router.push(`/tasks/detail/${item.id}`)} style={styles.card}><Text style={styles.cardTitle}>{item.title}</Text><Text style={styles.cardMeta}>{item.priority} priority · revision {item.revision}</Text></Pressable>} />
  </View>;
}
