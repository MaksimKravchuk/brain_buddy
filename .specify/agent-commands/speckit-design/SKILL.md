...
1. Read the repository's `brain-buddy-design` tokens and components at
   `.claude/skills/brain-buddy-design/`. This is a legacy storage path for
   shared design reference files, **not** a requirement to use Claude Code.
   Work in an isolated context when available; any agent or developer can
   author the design directly.
...
design work flagged as needing a person. Ask through the active agent's
human-interaction mechanism; never invent approval.
