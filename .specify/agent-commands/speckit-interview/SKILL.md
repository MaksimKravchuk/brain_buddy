...
Delegated workers may have no human-interaction channel. Human elicitation
must run in a session where the user can answer, so this stage **must** run in
...
Use the available human-interaction tool. One topic at a time. **There is no five-question cap** —
