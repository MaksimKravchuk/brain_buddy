...
Obtain an independent acceptance audit from a reviewer who did not implement
this feature; use a separate agent, developer or session. An implementer grading
its own work rationalizes, and this is the gate where that costs the most. If
the same session wrote the code, an independent reviewer is mandatory.

The reviewer writes two files:
