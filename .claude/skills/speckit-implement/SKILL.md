...
Delegate to the **`feature-implementer`** subagent. It works in an isolated
git worktree, which keeps product edits off the caller's tree and keeps the
long build/test transcript out of the caller's context.

For tasks marked `[P]`, prefer **fresh Claude Code Remote sessions** over
in-process parallel subagents. Two constraints make unbounded in-process
fan-out counterproductive here:
