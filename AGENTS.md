...
- Spec Kit owns versioned planning artifacts under `specs/` plus `.specify/`. Generated `tasks.md` is planning input, not permission to bypass isolated worktrees, TDD, independent review, CI, landing, or release gates. For a feature deliberately split into several PRs, agree on `specs/NNN-<slug>/delivery-slices.json` before coding; each PR gets only its named tasks, write paths, tests, branch and worktree. See `docs/spec-kit-workflow.md`.
