...
## Proportional Spec Kit Workflow
- Classify work by its effects before invoking Spec Kit; high-risk effects always win regardless of whether the change is described as a bug fix, maintenance, or refactor. Small maintenance, operational repairs, dependency/tooling upkeep, and behavior-preserving refactors use the owning Hermes card directly only when they do not touch a high-risk category and their intent is explicit. Standard feature work may use Spec Kit for `spec.md`/`plan.md` and an advisory review. High-risk work—auth, privacy, security, destructive data/schema changes, public contracts, migrations, concurrency, payments, safety/compliance, or irreversible effects—requires the bounded review and validated handoff before implementation.
- Spec Kit owns versioned intent and planning artifacts; Hermes Kanban remains the sole execution/orchestration and PR-review runtime. `speckit-implement` and `speckit-taskstoissues` are disabled. Generated `tasks.md` is planning input, not permission to bypass isolated worktrees, TDD, review, CI, merge, or release gates.
- A Spec Kit workflow, reviewer, timeout, malformed output, or handoff-generation failure must not create a separate process-gate card. Keep remediation and evidence on the owning feature card. Standard review failure does not block implementation when the owning card has explicit scope and acceptance evidence; record the unavailable review and continue. High-risk planning remains fail-closed, with one bounded rerun, until findings are resolved or the documented founder-acceptance path is used.
- Only genuine product or authority decisions may block for user input. Framework, database, API shape, module boundaries, test strategy, migration mechanics, reviewer runtime, retry policy, and handoff generation are technical decisions owned by the Architect.
- Before adding or changing a committed feature-spec package, run `python3 scripts/check_spec_kit_specs.py` (or `make check-specs`) and preserve documented grandfathering for historical specs. Follow `docs/spec-kit-workflow.md` and ADR-0011.
