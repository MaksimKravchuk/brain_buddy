# BrainBuddy Single-Start Extension for A2A — v1

**Identifier (URI)**: `https://github.com/MaksimKravchuk/brain_buddy/blob/main/docs/a2a-extensions/single-start/v1.md`
**Status**: Published. This file *is* the document the identifier above resolves to, and
A2A expects an extension's specification to be hosted at its URI
(`docs/topics/extensions.md`) — so this path is frozen by repository policy and must never
move, be renamed or be deleted. Every agent that declares the URI is pointing here, and
BrainBuddy links here from the duplicate-risk disclosure a user is asked to acknowledge; a
move turns both into a 404. A breaking change gets a new URI (`…/v2.md`), never an edit
here. `backend/tests/test_agent_a2a_extension_identifier.py` enforces this.

**Audience**: authors of A2A agents deciding whether to declare support, and anyone
auditing what "Guaranteed single start" means in BrainBuddy's interface.
