...
`BRAIN_BUDDY_FEATURE_FLAGS=delivery_canary=internal`; values are never printed), so the
deploy release seeds the admin smoke account and marks it internal. After deploying it
