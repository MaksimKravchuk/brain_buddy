...
- [ ] T056 [P] Add failing canonical proposal-patch/freeze/supersede/confirm tests proving each immutable selected action preserves target, before/after summaries, source cue, confidence/warnings, and destination; contains no result status/Task ID; and stays byte-stable while append-only immutable receipts project ordered pending/succeeded/failed/skipped results across parent-key conflict, partial failure, retry, and process restart in `backend/tests/test_brain_dump_confirmation_contract.py`.
- [ ] T057 [P] Add failing stored-payload migration and deprecated-alias overlap tests proving completed/cancelled v1 immutability/no replay; owner-serialized one-time active import as `legacy_preview_only`; preserved operation/segment/proposal IDs, title locks, and remove tombstones; deterministic synthetic patches/import marker; unavailable accurate reconciliation/retry; visible `provisional_only` warning; explicit `review-provisional` gate before separate freeze/confirm; direct `PATCH`/`commit` delegation without bypass; canonical/alias races; restart; and exact-one Tasks in `backend/tests/test_brain_dump_operation_migration.py`.
...
- [ ] T060 Add additive models/DTOs for complete immutable `ProposalBatch` action snapshots (target, before/after summaries, source cue, confidence/warnings, destination, no result fields), append-only immutable action receipts plus separate folded result projection, consent decisions, raw-audio retention, proposal revision, and `legacy_preview_only` import/provisional-review compatibility fields in `backend/app/modules/tasks/domain.py` and `backend/app/schemas/tasks.py`.
- [ ] T061 Implement persisted proposal revision, freeze invalidation, byte-stable batch creation, receipt-folded result projection, canonical confirm, deterministic child receipts, partial-result recovery, and owner-serialized one-time active-v1 import with preserved lineage/locks/tombstones and provisional-review gate in `backend/app/modules/tasks/service.py` and `backend/app/modules/tasks/repository.py`.
- [ ] T062 Implement canonical `review-provisional`, `POST .../proposals/{proposal_id}/patches`, `POST .../proposal-batches`, and `POST .../confirm` routes plus immutable-snapshot/receipt-derived-result and legacy warning projections in `backend/app/api/tasks.py`; no canonical Task may exist before confirm and no legacy freeze may bypass explicit provisional review.
- [ ] T063 Implement deprecated direct proposal `PATCH` and `/commit` strictly as adapters to the canonical service/records without bypassing provisional review, mark them deprecated/excluded from the mobile operation allowlist, and migrate `frontend/src/features/brain-dump/BrainDumpRoute.tsx` plus `BrainDumpRoute.test.tsx` to show/accept `provisional_only` before proposal-patch/freeze/confirm without extending the compatibility window.
...
- [ ] T067 Run T056–T059 plus existing Brain Dump recovery/reconciliation/API suites, prove one-time legacy import and provisional-review/confirmation across restart, regenerate OpenAPI, and prove deprecated aliases are absent from the mobile operation allowlist before unblocking mobile Voice work.

**Checkpoint**: Complete immutable review snapshots, receipt-derived results, canonical
patch/freeze/confirm, truthful provisional-only legacy import, current consent, and
retained-until/delete-now are persisted; migration/alias overlap cannot bypass review,
duplicate Tasks, or create them prematurely.
...
- [ ] T075 [P] [US3] Add failing review-screen tests for reconciled/provisional/conflicted states; visible non-dismissible legacy `provisional_only` warning and explicit review acceptance; canonical patch persistence; freeze invalidation/refreeze; complete immutable selected actions with separate receipt-derived results; conflict gate; discard; `Confirm N additions`; retained-until/delete-now/pending; no Add date/inference; and timeout replay in `mobile/__tests__/brainDump/ReviewScreen.test.tsx`.
...
- [ ] T086 [US3] Implement canonical proposal patch/freeze/refreeze/confirm, visible `provisional_only` review acceptance, immutable-action/receipt-derived per-action results, and retained-until/delete-now UI in `mobile/src/features/brainDump/ReviewScreen.tsx` and `mobile/app/(app)/brain-dump/[operationId]/review.tsx`.
