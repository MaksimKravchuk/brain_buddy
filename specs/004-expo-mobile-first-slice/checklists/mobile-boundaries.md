...
- [x] CHK047 Does mobile consume canonical append-only proposal patch → complete immutable action snapshot → confirm routes, with execution results projected only from append-only receipts while deprecated direct `PATCH`/`commit` aliases remain excluded from its generated operation allowlist? [Consistency, Contract §§Canonical proposal/Freeze/Confirm, ADR-0002]
- [x] CHK048 Are frozen-batch invalidation, deterministic child receipts, one-time `legacy_preview_only` import with preserved IDs/locks/tombstones, visible `provisional_only` warning, explicit review/confirmation, alias delegation without bypass, process restart, and partial failure covered before mobile Voice implementation? [Coverage, Plan §Canonical Voice backend prerequisite]
