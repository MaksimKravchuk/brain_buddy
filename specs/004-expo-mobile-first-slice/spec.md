...
A signed-in user records a foreground voice dump, consents separately to microphone access and external processing, safely stops and uploads the durable local recording, observes persisted processing/recovery state, reviews proposals, edits or removes them, and explicitly confirms the selected additions to Inbox.
...
**Independent Test**: Record a deterministic short utterance on a device, stop, resume after an app restart during upload or processing, remove one proposal, edit another, confirm, and prove exactly the confirmed title-only Inbox tasks exist after retry and web reload.
...
2. **Given** microphone permission but no current external-processing consent, **When** no local provider is configured, **Then** recording may not upload or invoke a provider and the limitation is visible.
...
6. **Given** sealing or provider processing, **When** the user waits or restarts the app, **Then** truthful stage, retry, cancellation, and correlation information is restored from the persisted operation projection.
7. **Given** proposals ready for review, **When** the user edits or removes items, **Then** those user decisions survive refresh/reconciliation and open conflicts block confirmation.
8. **Given** an eligible frozen proposal set, **When** the user confirms, **Then** only selected title-only Inbox additions are created, no due date/Project/Tag/Priority is inferred, and repeated confirmation cannot duplicate a Task.
9. **Given** partial commit or cancellation, **When** some action already succeeded, **Then** the app reports the actual per-operation result and does not promise an atomic rollback.
...
- **FR-014**: Microphone permission and external-processing consent MUST be separate, current, visible decisions. Audio MUST NOT leave the device when external processing is not permitted and no local provider exists.
...
- **FR-017**: The app MUST persist only the minimum local operation recovery metadata needed to bind the recording to its opaque owner ID, find the durable file, acknowledge chunks, preserve command keys/manifest, locate the server operation, and remember its last observed revision; raw content MUST not enter analytics or crash evidence.
...
- **FR-019**: Review MUST visibly distinguish provisional/reconciled/conflicted states, preserve user edits/removals, exclude tombstoned or superseded proposals, and block confirmation while conflicts remain.
- **FR-020**: Confirmation MUST create only explicitly selected title-only Inbox tasks through deterministic child idempotency. No Task may exist before confirmation.
...
- The existing Brain Dump backend remains bounded and does not yet claim full ADR-0002 event-stream, undo, retention-UI, or live-streaming conformance.
