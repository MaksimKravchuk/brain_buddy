...
- **FR-020**: Confirmation MUST target a persisted immutable `ProposalBatch` frozen at the selected proposal revision and create only its title-only Inbox actions through `H(operation_id,batch_id,action_id)` receipts. Mobile MUST NOT consume deprecated direct proposal `PATCH` or `/commit` aliases, and no Task may exist before canonical confirmation.
