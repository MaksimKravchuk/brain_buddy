...
These boxes record delivered outcomes, not the presence of this specification layer.
They stay unchecked here because no backend, web, mobile, or release artifact exists at
this layer; each is ticked only once the named evidence passes, consistent with the
unchecked task list in `tasks.md`.

- [ ] A user can connect/test an agent without retrieving saved secrets — evidence: passing FR-001–FR-003 backend and client tests.
- [ ] A reviewed hand-off sends one immutable payload once across ambiguous retries — evidence: SC-001 replay test and the SC-008 barrier-concurrency tests.
- [ ] Authenticated reports, blocked questions, replies, cancellation requests, terminal reports, silence, disconnect, and expiry remain distinguishable and honest — evidence: SC-002 and SC-005 state tests.
- [ ] Web and iOS expose the same server-owned projection and capability gates — evidence: web and iOS Task-surface tests against the same projection contract.
