...
**Feature Branch**: `007-external-agent-relay-lean-v1`

**Created**: 2026-08-09

**Status**: Baseline ratified; narrowly amended specification semantically approved; landing remains unapproved

**Ratification receipt**: On 2026-08-09 Max ratified the exact baseline bytes preserved in `ratified-baseline-2026-08-09.md`, SHA-256 `f3cf461ffa0340738b8d599d2c643a3979b7bea30fc08fabf90fa4c01299e384`, including the Design Acceptance Gate then present. Later amendments are distinguishable from that immutable baseline. On 2026-08-11 Max explicitly accepted the recorded procedural deviation and semantically approved the narrowly described amended specification; that approval is not a claim that Max approved the exact post-edit bytes. It does not authorize merge, deploy, rollout, or any other release action. Separate exact-SHA review, CI, and ASK merge authority remain required.
...
- Barrier-concurrent duplicate `start`, `reply`, or `cancel` submissions for the same
  owner-scoped intent converge on one durable winner, one stable run/command identity,
  and at most one connector-side action. Contenders return that winner's recorded or
  unconfirmed outcome; they never create parallel reservations or external actions.
...
- **FR-001**: BrainBuddy MUST let a signed-in owner create, test, inspect, update, rotate credentials for, and disconnect an owner-scoped external-agent connection using an agent name, an endpoint selected through an allowlisted server-owned connector adapter, and secret/authentication material. Owners MUST NOT select an arbitrary transport/tool mechanism. Hermes MUST be treated as a reference connector rather than a product dependency; additional connector adapters require an explicit governed server allowlist entry and the same security contract.
...
- **FR-004**: Ordinary users MUST be prevented from configuring loopback, link-local, metadata-service, or private-network destinations unless the deployment separately and explicitly enables that network class. Credential- or content-bearing traffic MUST use HTTPS with valid hostname and certificate verification under deployment policy. Scheme and destination validation MUST run before every resolution, redirect, retry, and subsequent outbound connection; insecure redirects and redirects or address changes into disallowed classes MUST be rejected before forwarding credentials or content. Each outbound socket MUST dial one address from the exact validated resolution result while retaining TLS certificate and hostname verification for the configured host; the transport MUST NOT perform a second unbound DNS lookup between validation and connection. Resolver-change/DNS-rebinding tests MUST prove that a later private or metadata answer cannot receive credentials or content.
...
- **FR-019**: Rollout `OFF` MUST block creating or updating a connection, every
  connection test, credential rotation or other credential mutation, hand-off preview,
  and fresh dispatch. It MUST NOT lock an authenticated owner out of owner-scoped
  connection/run reads or safe `reply`/`cancel` commands for an existing non-terminal
  run when the connector and current connection state support that command. Authenticated inbound
  reports for already-dispatched runs MUST continue while `OFF`; account purge and
  retention maintenance remain active independently of rollout state.
...
- **SC-008**: Implementation verification MUST include deterministic barrier tests for
  concurrent duplicate start, reply, and cancel submissions that prove one durable
  winner and one connector action for each command family.

## Design Acceptance Gate

The exact 2026-08-09 baseline required design work and human design approval before
architecture planning, generated tasks, handoff, or implementation. That sequence was
not followed and cannot be repaired retroactively. There is no missing design-preview
approval to infer or manufacture: on 2026-08-11 Max explicitly accepted this procedural
deviation together with the narrowly amended specification, without approving exact
post-edit bytes or authorizing merge or deploy.

The replacement landing gate is transparent and prospective. The actual current web
and iOS implementation, including the recovered task card/detail patterns, Connected
agents setup, capability and failure states, run lifecycle, and honest unsupported-
capability fallbacks, MUST pass final independent dual review on the exact candidate SHA,
the required CI on that same SHA, and separate ASK merge authority. Only those gates can
authorize landing; production deployment and rollout remain separately governed.
