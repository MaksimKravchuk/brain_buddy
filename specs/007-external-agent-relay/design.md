...
**Feature**: `specs/007-external-agent-relay/`
**Spec**: `spec.md` (the exact 2026-08-09 baseline is preserved as `ratified-baseline-2026-08-09.md`; later amendments remain distinguishable in the canonical file and provenance record)
**Screens**: Responsive product surfaces listed below; no versioned HTML prototype is stored in this repository
**Human sign-off**: Max ratified the exact 2026-08-09 baseline, including its Design Acceptance Gate. The required design-before-plan sequence was not met and cannot be retroactively fixed; no design-preview approval is claimed. On 2026-08-11 Max explicitly accepted that procedural deviation and semantically approved the narrowly amended specification, but did not approve exact post-edit bytes or authorize merge or deploy. The actual current web and iOS implementation still requires final independent dual review, CI on the exact candidate SHA, and separate ASK merge authority before landing.
...
|---|---|---|---|
| D-01 / M-01 | Add agent | Creates one owner-scoped connection after recent server-verified reauthentication at the account's highest enrolled assurance through the current server-supported mechanism | FR-001, FR-003 |
...
| D-01 / M-01 | Replace credential / signing secret | Uses one stable intent and one-time secret display after the same FR-003 reauthentication | FR-003, FR-006 |
...
### Requirements with no dedicated affordance

- FR-013, FR-015, and FR-017 are server projection/retention/correlation contracts with no control of their own; their effects and errors are visible but users do not invoke the rule.
- FR-004, FR-006, and FR-009 are server enforcement contracts that constrain the affordances mapped above rather than adding controls. The map cites them because Test connection, Replace credential, Confirm hand-off, Reply, and Request cancellation are governed by them — destination validation, run/idempotency reservation, and event authentication run on the server, and the user controls only the surrounding action.
...
## Procedural deviation and prospective gate

The baseline's sequence made approved design a prerequisite to planning and
implementation. Planning and implementation proceeded without that prerequisite, so
the historical condition is recorded as unmet rather than rewritten as satisfied. Max's
2026-08-11 acceptance ratifies the deviation and the narrow semantic amendment only.

The operative landing gate is evidence from the implementation that actually exists:
final independent web and iOS reviews bound to the exact candidate SHA, required CI on
that same SHA, and separate ASK merge authority. This record grants no merge, deploy,
rollout, ruleset, workflow, or release authority.
