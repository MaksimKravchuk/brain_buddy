...
**2026-08-15 founder correction** (spec.md DD-14/DD-15/DD-16, [ADR-0019](../../docs/decisions/0019-sqlite-runtime-feature-flag-store.md)): the section now covers three **managed flags** — `voice_brain_dump`, `mobile_task_classification` and `external_agent_relay` (spec.md DD-1). `delivery_canary` never appears on this screen: it is not runtime-manageable, and showing it without controls that work would be more confusing than not showing it at all. There is no more "deploy default"/inheritance state, and no more clear-override action — every flag's stored mode is always present, so `F-02`'s source note and "Use deploy default" action are retired below.
...
| F-02 | Flag list | the read succeeded | one row per managed flag (three, including `external_agent_relay` — DD-1, DD-16): name, current mode as a three-option native `fieldset`/`legend` radio group (Off / On / Selected users), always exactly one option checked — the flag's stored mode is always present, so there is no "no radio checked" state and no source note (FR-003, FR-010, DD-3 retired) |
...
- **Mode is a three-option control, not two toggles, and there is no separate clear/inherit action** (superseded 2026-08-15, DD-3/DD-15 retired). OFF/ON/SELECTED_USERS are mutually exclusive states (FR-003), modelled as a native `fieldset`/`legend` radio group so the screen can never render a state the server cannot store; exactly one option is always checked, because every managed flag's SQLite row always has an explicit mode.
- **Mutations are targeted, never a whole-form save.** Setting a mode, adding one account and removing one account are three separate operations (FR-005; superseded 2026-08-15 — was four, "clear an override" is retired). There is no "Save all," so two operators editing different flags never overwrite each other.
- **Exactly two named mutations require confirmation; the rest are immediate** (`F-08a`, DD-12, superseded 2026-08-15 — was three): setting a flag OFF, and removing the last remaining member — a fixed allowlist, not a computed "reduces exposure to zero" rule, unenforceable server-side against concurrent tabs. Every other mutation only adds capability or moves state the operator can already see and immediately undo, so it stays a single click — narrower than feature 009's confirm-on-revoke (irreversible, 009-FR-006); a mode change is always reversible by its own inverse.
...
- each managed flag's row stacks vertically: flag name, then the mode `fieldset` (its three radio options wrap to a second line rather than overflowing) — there is no source note or "Use deploy default" line to fit any more (superseded 2026-08-15, DD-3 retired);
