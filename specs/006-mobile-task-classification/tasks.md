...
- [x] T001 Develop on the designated feature branch. **The worktree was skipped, and this task had no standing to waive it.** What was done is left on the record as it was decided: the session's branch policy required this branch, a second worktree was judged to fight that, and the six parallel agents were isolated by disjoint file ownership rather than by directory. Two things in that original note are wrong. First, the isolated worktree is a **non-negotiable delivery gate**, not a default this file may trade away — `AGENTS.md` "Agent Delivery Workflow", `.specify/memory/constitution.md` "PRs are the merge gate", `.specify/templates/tasks-template.md` "Delivery gates", and this feature's own `plan.md` "Delivery boundary", which states the gate "remains authoritative. This plan changes none of that." A tasks.md line cannot grant itself the exemption; `scripts/check_gate_integrity.py` exists precisely because an actor relaxing its own conditions unnoticed is the failure mode. Second, "for no benefit" understates the trade: file-ownership isolation has its own blind spot, and `review-history.md` records that **all three** defects found during implementation lived at seams between the disjointly-owned files. That does not prove worktrees would have caught them, and it does not show the skip was harmless either. Recorded rather than rewritten, so the next feature does not re-derive this as precedent — an actual exemption needs a human decision, taken before the work, not a justification written beside it
...
- [x] T068 [US3] Implement the >24h drain rule in mobile/src/features/tasks/useClassificationQueue.ts: past `firstSentAt + 24h` the drain re-reads the task first, then takes one of **three** branches — drop the entry if the server already holds the intended value; re-present it against the current revision with a new key and refreshed `originalValue` if the server still holds what the phone last showed; or park it `conflicted` for M-04, `originalValue` unrefreshed, if the server holds a third value. **The third branch was missing as first delivered**, and re-presenting rebases `observedRevision` so the following send cannot 409 — which is the only thing that opens M-04 — so a change made against a task somebody else had reclassified overwrote their work silently, against FR-008 and SC-005. Corrected after founder acceptance; see `data-model.md` invariant 10 and `review-history.md`
