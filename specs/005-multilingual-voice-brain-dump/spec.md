...
**Status**: Delivered on branch `005-multilingual-voice-brain-dump` at HEAD `c979621` (US1-US3 core, FR-016 grounding tolerance with a documented fail-closed residue, the planning-review hardening lane, the commit-concurrency/audio-deletion fixes, and the commit_batch/committing retention fixes; 994 backend / 452 frontend green). This is a **founder-only INTERNAL** increment (single-user product) below ADR-0002's public-release targets. The live evidence report ran on the real pipeline at `900eeb8` (`run_key e8cb406f…`): SC-001/003/007 pass; SC-002 (74.4% strict) and SC-004 (2 conjunction splits) are below the PUBLIC-ON gates and explicitly accepted for founder-only use. Public ON exposure stays gated on the ADR-0002 targets.
...
Evidence status: the live evidence report ran on the real pipeline at commit `900eeb8` (report `run_key e8cb406f…`, committed at HEAD `c979621`; harness `scripts/voice_evidence_report.py`, schema v2, using the authoritative strict 4-signal SC-002 oracle). This is the current operational evidence. Two thresholds — SC-002 ≥80% and SC-004 zero conjunction splits — are **PUBLIC-ON gates** (ADR-0002-tier), not met yet; per the founder-only INTERNAL exposure decision the current live baseline is explicitly accepted for the founder's own use. The canonical corpus is `backend/tests/fixtures/voice_brain_dump/reference_corpus/founder_ru_reading_script.v1.json` (43 task-yielding utterances). Note: earlier ~82-88% figures used a looser correctness notion; the strict 4-signal oracle (74.4%) is authoritative.

- **SC-001** (met): the full capture→review→commit drive yields at least 15 committed tasks. (Live at `900eeb8`: **19 committed** — PASS; prior behavior: 0.)
- **SC-002** (PUBLIC-ON gate; founder-INTERNAL baseline accepted): at least 80% of the 43 task-yielding utterances yield exactly one **correct** task proposal under the strict 4-signal oracle (expected task count, required embedded foreign terms, intent/title equivalence, non-translation; `miss_breakdown`; inflection/grounded rewording is not a miss). (Live at `900eeb8`: **74.4% (32/43)**; `miss_breakdown` = 7 count_mismatch + 4 missing_required_terms. Below the 80% public gate; accepted for founder-only use.)
- **SC-003** (met): zero proposal titles are translated out of their source utterance's language; titles are language-faithful (inflection/grounded rewording is not a violation), measured by the corpus scorer with code-switched source-word fixtures kept separate from FR-008 morphology. (Live at `900eeb8`: **0/45 translated** — PASS.)
- **SC-004** (PUBLIC-ON gate; founder-INTERNAL baseline accepted): zero task-yielding utterances are split on a simple conjunction within one intent. (Live at `900eeb8`: **2/22** conjunction splits. Above the zero-split public gate; accepted for founder-only use.)
...
- **SC-006** (met): a consent set omitting a configured vendor fails closed with an explicit consent-mismatch error, and **no audio is persisted and no external-provider egress occurs** (first-party boundary). Primary evidence is the backend pre-persistence/pre-provider rejection test (delivered); the client capture gate is defense-in-depth. (Verified at HEAD `c979621`; a full-stack A+B/consent-only-B browser-egress assertion is a follow-up test, T043.)
- **SC-007** (met): end-to-end processing of a 4-minute recording completes within 2 minutes of sealing. (Live at `900eeb8`: **21.1 s** — PASS.)
- **SC-008** (met): full automated backend and frontend suites pass with the feature active. (Verified at HEAD `c979621`: **994 backend** / 452 frontend green; earlier 875/907/969 figures are superseded.)

## Assumptions

- The reference corpus is the versioned fixture `founder_ru_reading_script.v1.json` (a founder-authored, fictional Russian script with English terms, and its 4-minute m4a recording), representative of real capture behavior. Its authoritative ground truth: **43 task-yielding utterances** (25 create-category, plus existing-task modifications and multi-task utterances expecting 2-3 tasks), and 7 zero-task utterances (queries, display requests, fillers, "don't add"). This is the SC-002 denominator.
