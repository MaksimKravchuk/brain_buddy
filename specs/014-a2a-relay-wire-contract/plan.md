...
**Scale/Scope**: single-user accounts on a multi-tenant service; tens of concurrent non-terminal runs per deployment; two reference runtimes; six designed screens with 119+ states

## Constitution Check

*GATE: Must pass before Phase 0 research. Re-check after Phase 1 design.*

- **Spec workflow**: `spec.md` is final: the 2026-09-03 clarify session and the design sign-off decisions are encoded (zero `NEEDS CLARIFICATION` markers, `checklists/requirements.md` fully checked). `design.md` exists with human sign-off recorded; its open decisions were answered by the product owner and are reflected in FR-002, FR-003, FR-014 and AC-026. Re-check after Phase 1: `research.md` §Resolved and `data-model.md` carry the same answers. PASS.
...
├── src/features/agents/AgentHandoffOverlay.tsx              # D-02 states S01..S12 plus the acknowledgement and link states
...
├── src/features/agents/HandoffSheet.tsx                     # M-02 states S01..S11 plus acknowledgement and link states
