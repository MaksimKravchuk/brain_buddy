...
| --- | --- | --- | --- |
| 1 | Pull-request comment (updated in place, one per PR) | always posted, even when the gate is red | names both artifacts, marks `allure-report-single-file` as the starting point, says to unzip GitHub's envelope and open the single `index.html`, and states 7-day PR / 30-day push retention (008-FR-008) |
...
| 3 | Canary log line | pass: one line · fail: an explicit "THE GATE IS BROKEN" warning not to trust the run's verdict | 008-FR-005 |
